...
export function FinalCTAHT({ price, currencyCode, onAddToCart }: FinalCTAHTProps) {
  return (
      ...
      <div className="container-wide relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <div className="inline-flex items-center gap-2 bg-gold/20 backdrop-blur-sm border border-gold/30 rounded-full px-4 py-2 mb-6">
            ...
            <span className="text-gold text-sm font-medium">{ht.finalCta.badge}</span>
          </div>

          <h2 className="text-3xl md:text-4xl lg:text-5xl font-serif font-bold text-white mb-4">
            {ht.finalCta.title}<br />
            <span className="text-gold">{ht.finalCta.titleHighlight}</span>
          </h2>

          <p className="text-lg text-white/70 mb-10 max-w-2xl mx-auto">
            {ht.finalCta.subtitle}
          </p>

          {/* Value Stack */}
          <div className="bg-charcoal border border-charcoal-light rounded-2xl p-6 md:p-8 mb-8">
            <h3 className="text-lg font-semibold text-white mb-4">{ht.finalCta.valueStack.title}</h3>
            
            <div className="space-y-3 mb-6">
              {ht.finalCta.items.map((item, index) => (
            ...
            <div className="pt-4 border-t border-gold/30">
              <div className="flex items-center justify-between">
                <span className="text-lg text-white font-medium">{ht.finalCta.valueStack.totalValue}</span>
                <span className="text-2xl text-white/50 line-through">$508+</span>
              </div>
              <div className="flex items-center justify-between mt-2">
                <span className="text-lg text-gold font-bold">{ht.finalCta.valueStack.todayPrice}</span>
          ...
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-4 text-sm text-white/60">
              <span className="flex items-center gap-1">
                ...
                {ht.finalCta.trust.instantDownload}
              </span>
              <span className="flex items-center gap-1">
                <Clock className="h-4 w-4" />
                {ht.finalCta.trust.lifetimeAccess}
              </span>
              <span className="flex items-center gap-1">
                <Star className="h-4 w-4 text-yellow-400" />
                {ht.finalCta.trust.happyCustomers}
              </span>
            </div>
          </div>
